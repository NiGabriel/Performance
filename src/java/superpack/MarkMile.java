...
public class MarkMile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter();) {
            ...
            if (u == null) {
                ...
                out.print("<form action='RankMileEdit' method='post'>");
                ...
                try {
                    ...
                    PreparedStatement pst = con.prepareStatement("select midyearrating from report where goal = '" + myId + "' and staffname = '" + e.getStaffnamep() + "'");

                    ResultSet rs = pst.executeQuery();

                    if (rs.next()) {
                        String myrating = rs.getString("midyearrating");

                        out.print("Mid-year rating: <input type ='text' name ='midyrating' value ='" + myrating + "' class='form-control' style='width: 400px;' readonly />");

                        out.println("</div>");
                        out.println("<div class=\"mb-3\">");
                        out.print("End-year rating:<select name='endyrating' class='form-control' style='width: 400px;'> ");
