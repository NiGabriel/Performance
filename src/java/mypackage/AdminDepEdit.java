...
public class AdminDepEdit extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter();) {
            ...
            if (u == null) {
                ...
                out.println("<title>Admin - Edit Department</title>");
                out.println("</head>");
                out.println("<body>");

                String id = request.getParameter("siddp");
                int myId = Integer.parseInt(id);

                user e = UserDatabase.getDepById(myId);

                out.println("<div class=\"card\">");

                out.println("<div class=\"card-header\">\n"
                        + "			    <h2> Appraisal performance system - Modify department's data</h2>\n"
                        + "			</div>");
                ...
                out.print("<form action='DepEditServlet' method='post'>");
                out.println("<div class=\"mb-3\">");
                out.print("<input type ='hidden' name ='depid' value ='" + e.getDpid() + "'/>");
                out.println("</div>");
                out.println("<div class=\"mb-3\">");
                out.print("Name of department:<input type='text' name='depname' class=\"form-control\" style=\"width: 270px;\" value='" + e.getDpname() + "'/>");
                out.println("</div>");
