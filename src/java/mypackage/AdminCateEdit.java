...
public class AdminCateEdit extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter();) {
            ...
            if (u == null) {
                ...
                out.println("<title>Admin - Edit Category</title>");
                out.println("</head>");
                out.println("<body>");

                String id = request.getParameter("sidcate");
                int myId = Integer.parseInt(id);

                user e = UserDatabase.getCateById(myId);

                out.println("<div class=\"card\">");

                out.println("<div class=\"card-header\">\n"
                        + "			    <h2> Appraisal performance system - Modify category's data</h2>\n"
                        + "			</div>");
                ...
                out.print("<form action='CateEditServlet' method='post'>");
                out.println("<div class=\"mb-3\">");
                out.print("<input type ='hidden' name ='catid' value ='" + e.getCatid() + "'/>");
                out.println("</div>");
                out.println("<div class=\"mb-3\">");
                out.print("Name of category:<input type='text' name='catname' class=\"form-control\" style=\"width: 270px;\" value='" + e.getCatname() + "'/>");
                out.println("</div>");
